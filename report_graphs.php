...
<body>
    ...
    <div class="sidebar">
        ...
        <ul class="nav flex-column">
            ...
            <li class="nav-item">
                <a href="report_graphs.php" class="nav-link active">
    ...
    <!-- Main Content -->
    <div class="content">
        <h2 class="mb-4"><i class="fas fa-chart-line me-2"></i>Performance Graphs</h2>
        <?php if (isset($error)): ?>
                    ...
                    <div class="tab-pane fade show active" 
                        ...
                        <div class="tab-content" id="kraTabsContent-all">
                            <?php foreach ($kras as $kra_index => $kra): ?>
                                ...
                                foreach ($dept_counts as $dept_count) {
                                    $pareto_labels[] = $dept_count['department_name'] . ' (Sub)';
                                    $pareto_data[] = $dept_count['count'];
                                    $pareto_types[] = 'Submissions';
                                    $total += $dept_count['count'];
                                }
                                
                                // Non-submitters by department
                                foreach ($non_submit_counts as $non_submit) {
                                    if ($non_submit['non_submitters'] > 0) {
                                        $pareto_labels[] = $non_submit['department_name'] . ' (Non-Sub)';
                                        $pareto_data[] = $non_submit['non_submitters'];
                                        $pareto_types[] = 'Non-Submitters';
                                        $total += $non_submit['non_submitters'];
                                    }
                                }
                                ...
                                $cumulative = [];
                                $running_total = 0;
                                foreach ($pareto_data as $value) {
                                    $running_total += $value;
                                    $cumulative[] = $total > 0 ? ($running_total / $total) * 100 : 0;
                                }
                                ?>
                                <div class="tab-pane fade <?php echo $kra_index === 0 ? 'show active' : ''; ?>" 
                                    ...
                                    <div class="card">
                                        <div class="card-body">
                                            <canvas id="pareto-all-<?php echo $kra['kra_id']; ?>"></canvas>
                                        </div>
                                    </div>
                                    <script>
                                    const ctx_all_<?php echo $kra['kra_id']; ?> = document.getElementById('pareto-all-<?php echo $kra['kra_id']; ?>').getContext('2d');
                                    new Chart(ctx_all_<?php echo $kra['kra_id']; ?>, {
                                        type: 'bar',
                                        data: {
                                            labels: <?php echo json_encode($pareto_labels); ?>,
                                            datasets: [
                                                {
                                                    label: 'Count',
                                                    data: <?php echo json_encode($pareto_data); ?>,
                                                    backgroundColor: <?php echo json_encode(array_map(function($type) {
                                                        return $type === 'Submissions' ? 'rgba(54, 162, 235, 0.6)' : 'rgba(255, 99, 132, 0.6)';
                                                    }, $pareto_types)); ?>,
                                                    borderColor: <?php echo json_encode(array_map(function($type) {
                                                        return $type === 'Submissions' ? 'rgba(54, 162, 235, 1)' : 'rgba(255, 99, 132, 1)';
                                                    }, $pareto_types)); ?>,
                                                    borderWidth: 1,
                                                    yAxisID: 'y'
                                                },
                            ...
                            <div class="tab-content" id="kraTabsContent-<?php echo $dept['department_id']; ?>">
                                <?php foreach ($kras as $kra_index => $kra): ?>
                                    <div class="tab-pane fade <?php echo $kra_index === 0 ? 'show active' : ''; ?>" 
                                        ...
                                        <div class="card">
                                            <div class="card-body">
                                                <canvas id="chart-<?php echo $dept['department_id']; ?>-<?php echo $kra['kra_id']; ?>"></canvas>
                                            </div>
                                        </div>
                                        <script>
                                        const ctx_<?php echo $dept['department_id']; ?>_<?php echo $kra['kra_id']; ?> = document.getElementById('chart-<?php echo $dept['department_id']; ?>-<?php echo $kra['kra_id']; ?>').getContext('2d');
                                        new Chart(ctx_<?php echo $dept['department_id']; ?>_<?php echo $kra['kra_id']; ?>, {
                                            type: 'bar',
                                            data: {
                                                labels: ['<?php echo htmlspecialchars($kra['kra_name']); ?>'],
                                                datasets: [{
                                                    label: 'Submissions',
                                                    data: [<?php echo $report_data[$dept['department_id']]['kras'][$kra['kra_id']]['count']; ?>],
